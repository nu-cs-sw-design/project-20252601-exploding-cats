...
4) Excluding any Exploding Kitten and Defuse cards, the system shuffles the deck and deals 1 Defuse card and 4 other random cards from the deck to each player.
5) The system adds (N-1) Exploding Kitten cards into the remaining deck.
6) The system adds (6-N) Defuse cards back into the deck.
7) The system randomly shuffles the deck to create a draw pile.
8) The system randomly selects one player to take the first turn.
9) The system notifies the first player that it is their turn.

**Postcondition:** The beginning hands and draw pile for all players are established. The first player’s turn begins.\
**Alternate Flow:** If there are only 2 players, then only 2 Defuse cards are added to the remaining deck during step 6.
...
3)	The system prompts the active player to choose a position to re-insert the Exploding Kitten into the draw pile.
...
**Actor:** Player \
**Precondition:** The active player has played an action card that can be “Noped”.

**Basic Flow:**
1) The system detects that the active player has decided to play an action card that can be “Noped” (e.g. Shuffle, Skip, See the Future, another Nope, etc.)
...
**Postcondition:** All played ‘Nope’ cards are moved to the discard pile, and depending on the number of “Nope” cards in a chain, the original action card that was responded to with a “Nope” may or may not have been taken.